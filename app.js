...
function tabs(a){let x=[["dashboard","Tableau de bord"],["receipts","Réceptions"],["claims","Réclamations"],["stats","Statistiques"],["qr","QR universel"]];return `<div class="tabs noPrint">${x.map(([k,v])=>`<button data-tab="${k}" class="${a===k?"active":""}">${v}</button>`).join("")}</div>`}
...
async function reception(){
 let rating=5;
 A.innerHTML=`<section class="card hero">${logo()}<h1>Confirmer ma livraison</h1><p class="muted">Scannez, confirmez votre commande et partagez votre expérience de livraison.</p></section>
